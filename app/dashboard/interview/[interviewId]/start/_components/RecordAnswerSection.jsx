...
const RecordAnswerSection = ({
  ...
  // Consolidate speech-to-text results into a single transcript
  useEffect(() => {
  ...
  const StartStopRecording = async () => {
    if (isRecording) {
      ...
      console.log("Starting recording: Clearing previous results");
      setResults([]);
      setUserAnswer("");
      startSpeechToText();
    }
  ...
  const handleSaveAnswer = async () => {
    if (userAnswer.length < 10) {
      toast("Please record a longer answer before saving");
      return;
    }
    try {
      setLoading(true);
      ...
      console.log("Sending prompt:", feedbackPrompt);
      const result = await chatSession.sendMessage(feedbackPrompt);
      
      let responseText = await result.response.text();
      console.log("Raw response text:", responseText);
      
      // Clean up the response text (remove code block markers if any)
      responseText = responseText.replace("```json", "").replace("```", "");
      console.log("Cleaned response text:", responseText);
      
      const jsonFeedbackResp = JSON.parse(responseText);
      ...
      // Reset answer and results state after saving
      setUserAnswer("");
      setResults([]);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="md:flex mt-4 md:mt-8 md:gap-5">
        ...
        <Button onClick={handleSaveAnswer} disabled={loading || isRecording}>
          Save Answer
        </Button>
      </div>
    </div>
